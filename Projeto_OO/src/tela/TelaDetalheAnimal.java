...
/**
 * Implementa uma interface para mostrar os detalhes de cada animal cadastrado e permitir 
 * a edição desses dados, além de poder mostrar a interface de cadastro de um novo animal
...
public class TelaDetalheAnimal implements ActionListener {
	...
	private Font Arial = new Font("Arial Black",Font.BOLD,25);
	private JFrame f;
	...
	/**
	 * Cadastra novos animais e mostra detalhes dos já existentes, permitindo a edição desses dados
	...
	public void inserirEditar(int op,ControleDados d,TelaAnimal a,int pos) {
		...
		if (op == 1) s = "Cadastro de Ave";
		if (op == 2) s = "Cadastro de Cachorro";
		if (op == 3) s = "Cadastro de Gato";
		if (op == 4) s = "Cadastro de Vacina";
		if (op == 5) s = "Detalhe de Ave";
		if (op == 6) s = "Detalhe de Cachorro";
		if (op == 7) s = "Detalhe de Gato";
		if (op == 8) s = "Detalhe de Vacina";
		...
		botaoSalvar.setBounds(330,250,150,30);
		botaoExcluir.setBounds(100,250,150,30);
